...
V5 eliminates this split. **Hull construction, atlas, and particle movement operate in a single working space.** Seeds are L-stretched and radially lifted before hull construction; final positions are transformed back for output. The working space is Cartesian, so convex hulls, flat faces, and barycentric interpolation all remain valid. The radial lift and L-stretch serve four roles simultaneously:
...
**Repulsion energy** transitions from working-space Euclidean distances to **gamut-clipped OKLab distances** over the course of optimization via a metric continuation, aligning the final objective with the delivered result: perceptual distinguishability of the palette as the user will see it (Section 5.1).
...
The algorithm operates entirely in working space from this point forward. All geometric operations — dimensionality detection, hull construction, atlas building, particle movement — use working-space coordinates. The energy function transitions from working-space to clipped OKLab distances via metric continuation (Section 5.1).
...
**Handling:** 1D particle repulsion along the segment in working space, with energy transitioning from working-space to clipped OKLab distances via metric continuation (Section 5.1). Endpoint seeds are pinned. When mapped back through T⁻¹, the straight working-space segment becomes a curve in OKLab that bows outward from the L axis if it crosses the low-chroma region — providing automatic chroma preservation even in the 1D case — and spans a wider lightness range than the seeds (L-stretch effect).
...
- **Seeds wrapping around the hue wheel:** no face crosses gray. Full hull surface usable. The clipped-OKLab energy (Section 5.1) further adapts the distribution: particles avoid hull regions where gamut headroom is low, even if the surface area there is large.
- **All muted seeds:** the lift is gentle (r_s is small relative to seed chromas). Particles distribute using whatever separation the hull offers. The palette stays muted. Clipping is negligible, so the energy reduces to OKLab-distance repulsion.
...
The lift maps gray (r = 0) to the origin of the chromatic plane in working space. Because ρ'(0) = 0, a movable particle at exactly zero chroma in working space receives no chromatic gradient from the constraint geometry. It is a degenerate stationary point.
...
### 5.1 Riesz Energy on Clipped OKLab Positions

The repulsion energy blends two distance metrics via a **metric continuation** that transitions from working-space Euclidean distances (smooth, good for global exploration) to gamut-clipped OKLab distances (accurate, aligned with the output) over the course of optimization:

$$E_{\text{repulsion}} = \sum_{i < j} \frac{1}{\|d_i^{(ij)}\|^p}$$

where the effective distance between particles $i$ and $j$ is:

$$d^{(ij)} = (1 - \beta)\,\|p_i - p_j\| + \beta\,\|q_i - q_j\|$$

Here $p_i$ is the working-space position, $q_i = \text{clip}(T^{-1}(p_i))$ is the gamut-clipped OKLab position, and $\beta \in [0, 1]$ is the **metric blend parameter** that ramps from 0 to 1 during optimization (Section 8.2).

The **clipping function** $\text{clip}$ preserves hue and lightness, reducing chroma until the color is within sRGB gamut (Bottosson's gamut_clip_preserve_chroma). For in-gamut positions, $\text{clip}$ is the identity.

**Why two metrics, blended:**

The algorithm's goal is to maximize the worst-case perceptual distance of the delivered palette. Working-space distance is a proxy for perceptual distance; clipped OKLab distance *is* the perceptual distance of the output (up to OKLab's approximation). For seed configurations where most particles are in gamut (muted seeds, narrow-hue vivid seeds), the two metrics are similar and the choice has little effect. For seed configurations where the hull surface extends far beyond the sRGB gamut (vivid seeds spanning the hue circle), the proxy diverges from reality: particles at different working-space positions may clip to nearly identical OKLab colors, making them perceptually indistinguishable despite appearing well-separated in working space.

However, clipped-OKLab energy has weaker gradient signal than working-space energy when particles are far from the gamut boundary — the clip function flattens out the chroma dimension, reducing sensitivity to working-space displacement. Working-space Euclidean energy, by contrast, is smooth everywhere with strong gradients, making it better suited for the early phase of optimization where particles need to escape local minima and find their approximate global arrangement.

The metric continuation resolves this: at $\beta = 0$, the optimizer uses the smooth working-space landscape for global exploration. As $\beta$ ramps to 1, the optimizer transitions to clipped-OKLab distances, fine-tuning particle positions to maximize the actual output separation. This parallels the Riesz exponent continuation ($p: 2 \to 6$), and the two continuations run on the same schedule (Section 8.2).

**Self-adaptation property:** for in-gamut seed configurations, clip is the identity and $\|q_i - q_j\|$ equals $\|T^{-1}(p_i) - T^{-1}(p_j)\|$, which is close to $\|p_i - p_j\|$ when the lift is gentle (γ ≈ 1). In this regime the blend has little effect at any $\beta$, and the energy is equivalent to V5.1. The metric continuation only changes behavior when clipping is significant — precisely the configurations where V5.1 produced desaturated output.

**Continuation in p:** start at p = 2 for global exploration on a smooth energy landscape, ramp to p = 6 to sharpen focus on worst-case pair separation. This reduces local-minimum sensitivity in the early phase. The $\beta$ ramp runs on the same schedule (Section 8.2).

**Gradient computation:** use finite differences in working space. For each movable particle $i$, perturb each working-space coordinate by $\varepsilon$:

1. Compute the perturbed working-space position
2. If $\beta > 0$: map perturbed position through $T^{-1}$, gamut-clip the result
3. Recompute the blended distance to all other particles
4. Finite-difference the energy

At $\beta = 0$ (early iterations), only working-space distances are needed — no $T^{-1}$ or clip calls. At $\beta = 1$ (final iterations), only clipped OKLab distances are needed. At intermediate $\beta$, both are computed. Cost at $\beta > 0$: 3 extra $(T^{-1} + \text{clip})$ evaluations per movable particle per iteration. Both $T^{-1}$ and $\text{clip}$ are closed-form — negligible cost.

**Gradient through clipping:** moving a particle on the hull surface changes the hue angle $\theta$ and lightness $L$ of the pre-clip OKLab position. These change the clip point smoothly — the gamut boundary chroma $r_{\max}(\theta, L)$ varies continuously with $\theta$ and $L$. The finite-difference gradient captures this sensitivity. The pre-clip chroma has no effect on the clipped position (as long as it exceeds $r_{\max}$), so the gradient signal comes entirely from $\theta$ and $L$ variation. This signal is nonzero and directionally meaningful: it points toward hull regions where the gamut boundary offers higher chroma.
...
**Why retain E_gamut alongside clipped repulsion:** the two terms serve complementary roles. $E_{\text{repulsion}}$ optimizes the perceptual separation of the output; it cares about where particles clip to, not how far they are from the gamut. $E_{\text{gamut}}$ provides a smooth gradient pulling particles toward the gamut boundary, preventing them from drifting far outside where the clip function produces weak gradient signal (because $\theta$ and $L$ change slowly relative to working-space displacement on far-out-of-gamut hull faces). Together, $E_{\text{gamut}}$ keeps particles near the gamut, and $E_{\text{repulsion}}$ distributes them optimally within that constraint.
...
$E_{\text{repulsion}}$ is piecewise smooth: smooth within each hull face (clipping is a continuous operation with a continuous derivative almost everywhere), with face transitions introducing non-smooth boundaries. $E_{\text{gamut}}$ is C¹ continuous.

The **constrained** problem — particles on a polyhedral hull surface with face transitions and boundary clamping — is **piecewise smooth**, not globally smooth. Gradient-based optimization works well on piecewise smooth problems; the face transitions are handled explicitly by the atlas.
...
Loop until convergence:
  1. If β > 0: compute clipped OKLab positions for all particles
     (map through T⁻¹ → OKLab → clip to sRGB gamut)
  2. Compute repulsion gradient for each movable particle:
     finite differences using blended distance
     d = (1−β)·‖p_i−p_j‖ + β·‖q_i−q_j‖ with current Riesz exponent p
  3. For out-of-gamut particles only:
     compute gamut penalty gradient via finite differences in working space
     (T⁻¹ → linear RGB → quadratic penalty)
  ...
  8. Update p and β according to continuation schedule (Section 8.2)
  9. Check convergence (see Section 8.3)

Output: Final working-space positions for all particles
```

The optimization loop contains no warp Jacobians and no pullbacks. Particle movement uses the face atlas in working space. During early iterations ($\beta = 0$), the energy is plain working-space Euclidean — no $T^{-1}$ or clip calls needed for repulsion. As $\beta$ ramps to 1, the energy transitions to clipped OKLab distances, requiring $T^{-1}$ and clip evaluations via the closed-form inverse lift and gamut clipping. Both the repulsion and gamut penalty gradients use finite differences — trivially correct, negligible cost.
...
**Metric blend β:** ramp from β_start = 0 to β_end = 1 on the same schedule as the Riesz exponent. The two continuations are coupled: both transition from "smooth/exploratory" (low p, working-space metric) to "sharp/accurate" (high p, clipped-OKLab metric). Using the same schedule avoids an additional tuning parameter and ensures that the energy landscape transitions coherently — the metric sharpens alongside the exponent.
...
The fraction of output colors requiring clipping depends on the seed configuration. The algorithm does not guarantee all optimized positions are exactly in gamut; it guarantees that the penalty strongly discourages out-of-gamut positions and that final clipping produces valid sRGB output. Because the repulsion energy transitions to clipped positions at $\beta = 1$ (Section 5.1), the final optimization phase already operates on the clipped distances — final clipping does not change the pairwise distances that were optimized in the last stage.

### 9.3 What the Output Represents

- **Optimization domain:** hull surface in working space
- **Optimization objective:** pairwise separation, transitioning from working-space distances to clipped OKLab distances via metric continuation
...
| Metric blend start | β_start | 0 | Working-space Euclidean for global exploration |
| Metric blend end | β_end | 1 | Clipped OKLab for output-aligned fine-tuning |
...
- Repulsion energy converges to actual output distances: via metric continuation, the final optimization phase measures pairwise separation on the clipped OKLab colors the user receives
...
- Gamut-aware distribution: particles avoid hull regions where gamut headroom is low, concentrating on positions that yield vivid output
...
- **Smooth energy gradient through clipping:** the gradient of $E_{\text{repulsion}}$ through the clip function is computed via finite differences. The clip function is continuous but not everywhere differentiable (it has kinks at gamut boundary transitions). Finite differences handle this correctly in practice.
...
- Per iteration: O(N²) for pairwise forces; + O(N) clip evaluations when β > 0
...
| Full hue wraparound | 4–6 seeds evenly around hue wheel | Closed hull, clipped-energy distribution, gamut headroom |
| Muted anchor | 1 warm gray + 4 vivid seeds | r_s robustness, interior seed handling |
| All muted | 4 low-chroma seeds | Lift gentleness, palette stays muted, clipped ≈ lifted regression |
...
| Vivid rainbow | 6–7 vivid ROYGBIV seeds | Clipped-energy effect: particles prefer high-headroom positions. Compare min ΔE vs V5.1. |
| Clipped-energy regression | 4 muted earth tones | 0% clipping → clipped energy ≈ OKLab energy. Must not regress from V5.1. |
| Clipped-energy gradient | 3 vivid triadic seeds, 8 free | Gradient signal through clip: particles should cluster near seed hues, not at face centroids. |

Measure: minimum pairwise ΔE in OKLab, distribution of pairwise distances, fraction requiring final gamut clipping, subjective family coherence, effective γ computed by adaptive formula, lightness range of output palette vs. seed lightness range, and mean clipped chroma of free particles.
...
### 13.3 What Changed from V5.1 to V5.2

| Aspect | V5.1 | V5.2 |
|--------|------|------|
| Repulsion distances | Working-space Euclidean | Blended: working-space → clipped OKLab via metric continuation (β: 0→1) |
| Repulsion gradient | Analytical Euclidean (closed-form) | Finite differences through blended metric |
| Gamut awareness in repulsion | None (separate penalty only) | Intrinsic at β > 0: output distances enter the objective |
| Continuation schedules | p: 2→6 | p: 2→6 coupled with β: 0→1 |
| Per-iteration cost (early, β=0) | O(N²) distances | O(N²) distances (identical) |
| Per-iteration cost (late, β=1) | O(N²) distances | O(N²) distances + O(N) clip evaluations |
| Effect on muted seeds | — | None (clip = identity at all β) |
| Effect on vivid wide-hue seeds | Desaturated intermediates | Particles migrate to high-headroom positions |

### 13.4 What Stayed the Same (V4.4 → V5.0 → V5.1 → V5.2)
...
- All lift parameters (γ, r_s, R, vividness, spread)

### 13.5 Why V5.0 Changed from V4.4
...
### 13.6 Why V5.1 Changed from V5.0
...
### 13.7 Why V5.2 Changed from V5.1

V5.1 computes repulsion energy on working-space Euclidean distances. This works well when the working-space hull surface stays within or near the sRGB gamut, because lifted-space distances approximate OKLab distances (up to the lift distortion). However, for vivid seeds spanning the hue circle (Δh_max > 120°, γ > 2), the radial lift pushes the hull surface far beyond the sRGB gamut. After inverse-lifting and gamut clipping, hull face centroids lose 50–80% of their chroma. In this regime, working-space distance is a poor proxy for the output's perceptual distance: particles at different hull positions clip to nearly identical desaturated colors.

The root cause is a proxy mismatch. The optimizer maximizes separation in working space, but the user receives gamut-clipped OKLab colors. When clipping is significant, these can diverge — the optimizer declares success while the output contains near-duplicate pastels.

V5.2 closes this gap with a **metric continuation**: the repulsion energy blends working-space Euclidean distances ($\beta = 0$, smooth landscape for global exploration) with clipped OKLab distances ($\beta = 1$, output-aligned for fine-tuning), ramping $\beta$ from 0 to 1 on the same schedule as the Riesz exponent $p$. At the end of optimization, the energy is computed entirely on clipped OKLab positions — the optimizer maximizes what actually matters: the perceptual separation of the delivered palette. For configurations with negligible clipping (muted seeds, narrow-hue vivid seeds), the blend has no effect at any $\beta$ — clipped positions equal OKLab positions, and the energy is equivalent to V5.1. For configurations with significant clipping (vivid rainbow, complementary pairs), particles redistribute toward hull regions with higher gamut headroom.

**Quantitative evidence (1D, Red ↔ Cyan edge, 5 free particles, p = 6):**

| Energy metric | min pairwise ΔE (clipped OKLab) |
|---------------|:-------------------------------:|
| Working-space Euclidean (V5.1) | 0.057 |
| Clipped OKLab (V5.2) | 0.091 |

V5.2 improves worst-case distinguishability by **+60%** on this edge case, with zero effect on muted seeds (0% clipping → identical behavior).
...
- **R1 §3.1 (metric inconsistency):** resolved — geometry and movement in working space; energy transitions from working-space to clipped OKLab via metric continuation.
- **R1 §3.2 (lightness untouched):** retained intentionally. See Section 4.4.
- **R1 §3.3 (objective mismatch):** Riesz energy with continuation retained. Energy now computed on output distances. See Section 5.1.
...
- **R5 §2 (warp Jacobian pullback):** eliminated — no Jacobian in the optimization loop.
...
### Round 10: V5.2 Clipped-OKLab Repulsion Energy

#### 14.8 Metric continuation: working-space → clipped OKLab repulsion

**Status: New in V5.2.** Repulsion energy transitions from working-space Euclidean distances to gamut-clipped OKLab distances via a metric blend parameter $\beta$ that ramps from 0 to 1 on the same schedule as the Riesz exponent $p$. This couples two complementary continuations: the exponent continuation ($p: 2 \to 6$) sharpens the energy from smooth global exploration to worst-case pair focus; the metric continuation ($\beta: 0 \to 1$) transitions from the smooth working-space landscape to the output-aligned clipped-OKLab landscape.

At $\beta = 0$ (early iterations), the energy is plain working-space Euclidean — identical to V5.1, no $T^{-1}$ or clip calls required. At $\beta = 1$ (final iterations), the energy uses clipped OKLab distances, aligning the objective with the delivered palette. The coupling avoids an additional tuning parameter: both $p$ and $\beta$ serve the same architectural purpose (exploration → exploitation) and share a schedule.

The change is motivated by analysis showing that for vivid wide-hue seeds, working-space distance is a poor proxy for output perceptual distance: hull face centroids lose 50–80% of their chroma after inverse-lift and gamut clipping, but working-space energy cannot detect this.

For in-gamut seed configurations, clip is the identity at all $\beta$, so the metric continuation has no effect and the energy is equivalent to V5.1 throughout. The change only matters when clipping is significant — precisely the configurations where V5.1 produced desaturated output.

#### 14.9 E_gamut retained alongside clipped repulsion

**Status: Confirmed in V5.2.** The gamut penalty $E_{\text{gamut}}$ is retained. It serves a different role from the clipped repulsion: $E_{\text{gamut}}$ provides a smooth gradient pulling particles toward the gamut boundary, while $E_{\text{repulsion}}$ optimizes the distribution of output colors. Without $E_{\text{gamut}}$, particles far outside the gamut would experience weak repulsion gradients (because $\theta$ and $L$ change slowly with working-space displacement on far-out-of-gamut hull faces).

#### 14.10 Rejected alternatives for the vivid desaturation problem

Several alternative approaches were investigated and rejected before arriving at the metric continuation to clipped-OKLab repulsion:

- **Gamut-aware adaptive spread:** only addresses L-stretch, not the fundamental proxy mismatch.
- **Post-optimization chroma projection:** particles are already at the gamut boundary after clipping. No effect.
- **Gamut-normalized lift (R(L)):** mixes gamut-specific lookup tables into the lift geometry; breaks joint convexity and Jensen's inequality guarantee.
- **Chroma-loss energy penalty:** quasi-redundant with $E_{\text{gamut}}$; pushes particles toward seeds, conflicting with Riesz repulsion.
- **Nonlinear L-warp:** addresses only L extremes, ignores hue-dependent gamut topology.
- **Energy on pre-clip OKLab positions:** changes muted palettes more than vivid ones (ratio spread 2.80× vs 1.91×) — higher regression risk for the case that already works. Does not account for the actual gamut ceiling.
- **Pure clipped energy from iteration 0:** clipped-OKLab energy has weaker gradients than working-space energy when particles are far from the gamut boundary. Using it from the start risks poor global exploration. The metric continuation ($\beta: 0 \to 1$) resolves this by starting with the smooth working-space landscape.
- **Energy on clipped positions with analytical gradients:** the gamut boundary in OKLab is non-smooth (ridges at primaries). Finite differences handle this naturally; analytical differentiation of the clip function would be fragile.

The metric continuation avoids all these issues: it aligns the final objective with the output, preserves smooth global exploration in the early phase, adds no user-facing parameters, does not modify the lift geometry, has zero effect on in-gamut configurations, and uses the same finite-difference gradient technique already proven for $E_{\text{gamut}}$.
...
[3b] Seed Classification (hull-vertex / boundary / interior in working space)
         ↓
[4] Energy Function (depends on SpaceTransform interface for T⁻¹ + clip)
         ↓
[5] Initialization (pin seeds, exact face areas, sampled greedy placement, gray jitter)
         ↓
[6] Optimization Loop (blended forces → tangent projection → displacement → edge crossing → anneal p,β)
         ↓
...
| Energy function | 50 | Blended Riesz (working-space + clipped-OKLab) + gamut penalty, finite differences |
| Initialization | 60 | Exact areas, sampled greedy placement, jitter |
| Optimization loop | 80 | Finite-diff forces, metric continuation (β), edge crossing, annealing |
| 1D specialization | 30 | Line segment repulsion |
| Output | 25 | T⁻¹, gamut clip, sRGB conversion |
| **Total** | **~490** | Excluding library dependencies |

---

## 16. Summary

Facette generates perceptually distinct color palettes by treating the problem as particle repulsion on the convex hull surface of user-defined seed colors in a lifted OKLab space. The space lift combines a convex radial chroma transform with an affine lightness stretch, serving four roles simultaneously: it contracts the low-chroma region (gray avoidance), guarantees chroma preservation on intermediate colors (Jensen's inequality), defines the palette family (hull in working space), and expands lightness diversity (L-stretch). Particle movement uses the face atlas in working space, where faces are genuinely flat. A metric continuation transitions the repulsion energy from working-space Euclidean distances (smooth, for global exploration) to gamut-clipped OKLab distances (output-aligned, for fine-tuning), coupled with the Riesz exponent ramp on a shared schedule.
...
For in-gamut seed configurations, the metric continuation has no effect — clipped positions equal OKLab positions, and the energy is equivalent to previous versions. For vivid seeds spanning the hue circle, where the hull surface extends beyond the sRGB gamut, the clipped energy redistributes particles toward hull regions with high gamut headroom — preventing the desaturated intermediates that arose from optimizing a proxy that diverged from reality.

**The lift defines the family. The convexity preserves the vividness. The stretch extends the lightness. The metric continuation aligns the objective with the output.**
